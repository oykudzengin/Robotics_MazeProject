...
#define K_ATT_DRIVE 1000.0
#define K_REP_DRIVE 0.05
#define NO_EFFECTION_POT_FIELDS_DRIVE 0.22
#define ROT_RATE_DRIVE 0.1
#define BASE_SPEED_DRIVE 8.0
#define WALL_THRESHOLD_DRIVE 700
...
void particle_odometry_update(const LikelihoodField &lhf,
    ...
    for (auto &p: particles) {
        ...
        if (dist > 0.1) {
            ROS_WARN(
                "Particle moved by %f, rot1 is %f, rot2 is %f, trans is %f, rot1_noise is %f, rot2_noise is %f, trans_noise is %f",
                dist, delta_rot1, delta_rot2, delta_trans, delta_rot1_hat, delta_rot2_hat, delta_trans);
        }
